...
public class packed extends AbstractNumericCoded
{
	...
	*/
	...
	public packed(int length, int scale, pointer overlay)
	{
	...
	{
		this(length, scale);
		assign(numeric.newBigDecimal(value));
	}
	/**
	 * Create a packed decimal value with the specified length and scale initialized
	 ...
	 */
	public FixedData move(packed value)
	{
	 ...
	 */
	public FixedData movel(packed value)
	{
		//return super.move((FixedData) value);
		// If the value that is being moved is >= the length of this value and has the same scale 
		// then we can just copy bytes.
	...
	{
		int zLength = len();
		zoned z = new zoned(zLength, Scale);
