 ...
 * @author Andrew Clark
 */
public class fixed extends FixedData
{
	 ...
	 */
	fixed(int sz, byte array[], int index)
	{
	 ...
	 * @param overlay
	 */
	public fixed(int sz, pointer overlay)
	{
		super(sz, overlay);
	}
	 ...
	 */
	public void assignr(fixed fStr)
	{
		fStr.rSubfields();
		int msize = msize();
 ...
 */
	public void cat(fixed t2, int blanks)
	{
		catStr(t2.getOverlay(), t2.getOffset(), t2.checkr(' '), blanks);
	}
/** Concatenate another string to the blank-trimmed value of this fixed-length string. 
 * @see #cat(fixed t2, int blanks)
 */
	public void cat(fixed t2, INumeric blanks)
	{
 ...
 * @param start The 1-based index to begin checking from.
 */
	public int check(fixed fStr, int start)
	{
		fStr.rSubfields();
		rSubfields();
		start--;
 ...
 * Beginning at <code>start</code>, return the left-most position of this fixed-length string that is not set to any of the 
 * values in the specified string.
 * @see #check(fixed fStr, int start)
 */
	public int check(fixed fStr, INumeric start)
	{
 ...
 * @param start The 1-based index to begin checking from.
 */
	public int checkr(fixed fStr, int start)
	{
		rSubfields();
		fStr.rSubfields();
		start--;
 ...
 * Beginning at <code>start</code>, return the right-most position of this fixed-length string that is not set to 
 * any of the values in the specified string.
 * @see #checkr(fixed fStr, int start)
 */
	public int checkr(fixed fStr, INumeric start)
	{
 ...
 */
	...
	private int compareTo(fixed fStr)
	{
		rSubfields();
		fStr.rSubfields();
		int i = 0;
	...
	public boolean equals(fixed str)
	{
	 ...
	 * @return the 1-based index of the occurrence of the search value, or 0 if none found. 
	 */
	public int scan(fixed fStr, int startIndex)
	{
		fStr.rSubfields();
		rSubfields();
	 ...
	 * @return the 1-based index of the occurrence of the search value, or 0 if none found. 
	 */
	public int scan(fixed fStr, INumeric startIndex)
	{
	...
	/** 
	 * Scan this fixed length string for the occurrence of another string. 
	 * @see #scan(fixed fStr, int startIndex)
	 */
