...
public class timestamp extends date {
	...
	private timestamp(String datfmt, String datfmt400)
	{
	 ...
	 */
	public timestamp(pointer overlay)
	{
	 ...
	 */
	timestamp(String datfmt, int length, String datfmt400) {
	...
	{
		timestamp ts = new timestamp("yyyyMMddHHmmssSSS", datfmt400);
		try
		{
			ts.assign(value);
		}
	...
	public static void main(String[] args) throws Exception
	{
		...
		ts2.assign(ts);
		date d = new date("*MDY", "03/19/70");
		ts2.assign(d);
		time t = new time("*HMS", "05:06:03");
		ts2.assign(t);
		ts2.adddur(5, Application.DAYS);
		d.assign(ts2);
		ts2.adddur(8, Application.HOURS);
		t.assign(ts2);
		packed p = new packed(7, 0 , 1234567);
		ts2.move(p);
		zoned z = new zoned(5,0,19990);
		ts2.movel(z);
